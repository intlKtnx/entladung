                    ...
                    for samplename, group2 in tqdm(group.items()):
                        if gname == '#refs#':
                            if not len(group2) == 5:
                                ...
                                ds = np.array(group2[4])
                                maximum = np.max(ds)
                                minimum = np.min(ds)
                                ds = [x == (x - minimum)/(maximum - minimum) for x in ds]
                                if isinstance(group2[2], np.ndarray):
                                    # logging.info(f"{samplename}_{datensatz_name}")
                                    if datensatz_name == "mit":
                                        # plt.plot(group2[4])
                                        neg_grenzflache.create_dataset(name=f"{samplename}_{datensatz_name}", data=group2[4])
                                        neg_grenzflache_sum += 1
                                    elif datensatz_name == "ohne":
                                        # plt.plot(group2[4])
                                        neg_spitze.create_dataset(name=f"{samplename}_{datensatz_name}", data=group2[4])
                                        neg_spitze_sum += 1
                    # plt.title(str(h5_dataset_fp))
                    # plt.show()
    ...
    for h5_dataset_fp in files:
        with h5py.File(h5_dataset_fp) as h5_file:
            ...
            for i in tqdm(range(laenge)):
                ...
                #logging.info(index)
                ds = data_one_dimension[0, i, :]
                maximum = np.max(ds)
                minimum = np.min(ds)
                ds = [(x - minimum )/(maximum -minimum) for x in ds]
                if datensatz_name == "mit":
                    pos_grenzflaeche.create_dataset(name=f"{index}_{datensatz_name}", data=ds)
                    pos_grenzflaeche_sum += 1
                    # plt.plot(data_one_dimension[0, i, :])
                elif datensatz_name == "ohne":
                    pos_spitze.create_dataset(name=f"{index}_{datensatz_name}", data=ds)
                    pos_spitze_sum += 1
                    # plt.plot(data_one_dimension[0, i, :])
            # plt.title(datensatz_name)
            # plt.show()
...
if __name__ == '__main__':
    ...
    toh5("/home/marcus/Dokumente/entladung/data", "normalized_rnn_data_.h5")
